...
------------------------------------------------------------------------------------------------
UpdateList
2020/04/09:1.修改Sample、UpdateList文字描述 2.微調progressContent(進度條文字)的樣式 3.修正IE上傳檔案時的錯誤(IE在清空上傳元件時會觸發change導致檔案被清空，改成清空元件時先去把change事件移除)
2019/11/06:1.修正KYUploadSend沒設定事件參數會導致事件遺失的錯誤
2019/10/18:1.上傳時可以傳入除了change以外的事件複寫要觸發的function了
...
(function ($) {
    ...
    $.fn.KYUpload = function (options) {
        ...
        if (showBar) {
            ...
            progressHTML += '<div class="progressContent" style="width:0; height:100%; white-space: nowrap; background-color:#09F; text-align:center; font-size:20px; font-weight:bold;color: #8e0000;"></div>';
            progressHTML += '</div>';
